...
# Local-only overlay: drop in a copy of each fuzz_*.cc shipped under
# $SRC/harnesses/ so the locally-built fuzzers exercise the same source as
# the one being proposed upstream. The upstream PR itself replaces the
# original file; this overlay only matters until that PR lands.
if [ -d "$SRC/harnesses" ]; then
    for h in "$SRC/harnesses"/fuzz_*.cc; do
        if [ -f "$h" ]; then
            cp "$h" pdns/
        fi
    done
...
# copy extra seeds prepared in $SRC/seeds (see Dockerfile)
if [ -d "$SRC/seeds" ]; then
    for sub in raw-dns-packets txt-records zones proxy-protocol-raw-packets http-raw-payloads; do
        if [ -d "$SRC/seeds/$sub" ]; then
            mkdir -p "fuzzing/corpus/$sub"
            cp -r "$SRC/seeds/$sub/." "fuzzing/corpus/$sub/"
        fi
    done
fi
